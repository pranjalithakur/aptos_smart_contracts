module aptos_friend_addr::aptos_friend {
    ...
    #[view]
    public fun get_user_portfolio(user_addr: address): vector<(address, u64)> acquires User, Holding {
        let user_obj_addr = get_user_obj_addr(user_addr);
        if (!exists<User>(user_obj_addr)) {
            return vector::empty();
        };

        let user = &User[user_obj_addr];
        let out = vector::empty<(address, u64)>();

        let i = 0;
        while (i < vector::length(&user.holdings)) {
            let holding_obj_ref = vector::borrow(&user.holdings, i);
            let holding = &Holding[object::object_address(holding_obj_ref)];
            vector::push_back(&mut out, (holding.issuer, holding.shares));
            i = i + 1;
        };
    ...
    #[view]
    public fun get_user_total_position_value(
        user_addr: address
    ): (u64, u64, u64) acquires User, Holding, Issuer {
        let user_obj_addr = get_user_obj_addr(user_addr);
        if (!exists<User>(user_obj_addr)) {
            return (0, 0, 0);
        };
        ...
        let user = &User[user_obj_addr];
        let total_positions = vector::length(&user.holdings);
        ...
        while (i < total_positions) {
            let holding_obj_ref = vector::borrow(&user.holdings, i);
            let holding = &Holding[object::object_address(holding_obj_ref)];
            let issuer_obj = get_issuer_obj(holding.issuer);
            let amount = holding.shares;

            let (share_cost, issuer_fee, protocol_fee, _total_cost) =
                calculate_sell_share_cost(issuer_obj, amount);

            total_shares = total_shares + amount;
            total_sell_value = total_sell_value + share_cost;
            total_fees = total_fees + issuer_fee + protocol_fee;

            i = i + 1;
        };

        (total_shares, total_sell_value, total_fees)
    }
